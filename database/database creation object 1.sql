...
CREATE TABLE IF NOT EXISTS `ctam`.`runs_in_experiment` (
  ...
  `role` VARCHAR(45) NOT NULL COMMENT 'role column describes the purpose/role of the run in the experiment, e.g. control, 24h treatment',
...
CREATE TABLE IF NOT EXISTS `ctam`.`peptide` (
  ...
  `phospholyration_site` VARCHAR(255) NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `ctam`.`final_result` (
  ...
  PRIMARY KEY (`peptide_id`, `experiment_id`, `treatment_id`),
  INDEX `experiment_final_result_idx` (`experiment_id` ASC),
  INDEX `treatment_final_result_idx` (`treatment_id` ASC),
  CONSTRAINT `peptide_final_result`
    FOREIGN KEY (`peptide_id`)
    REFERENCES `ctam`.`peptide` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `experiment_final_result`
    FOREIGN KEY (`experiment_id`)
    REFERENCES `ctam`.`experiment` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `ctam`.`observed_RT` (
  ...
  PRIMARY KEY (`experiment_id`, `run_id`, `peptide_id`),
  INDEX `peptide_calculated_RT_idx` (`peptide_id` ASC),
  CONSTRAINT `experiment_and_run_observted_RT`
    FOREIGN KEY (`experiment_id` , `run_id`)
    REFERENCES `ctam`.`runs_in_experiment` (`experiment_id` , `run_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `ctam`.`experiment_cell_line` (
  ...
  CONSTRAINT `experiment_experiment_regulator0`
    FOREIGN KEY (`experiment_id`)
    REFERENCES `ctam`.`experiment` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
